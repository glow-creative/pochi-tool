...
function updateHistoryButtons() {
  ...
  // Nothing to go back to before the first edit, and nothing to go back to
  // once the picture is already the one that was opened.
  elements.resetButton.disabled =
    state.documentPaper === null || state.currentRevision === 0;
}
...
// Back to the picture as it was opened, and the steps that led away from it go
// with it. Keeping them would mean holding the opened picture and a way back
// through every edit at once, and on a large picture there is only room for a
// few pictures in all - so the way back is the opened picture itself.
function resetToOpenedPicture() {
  if (state.documentPaper === null || state.currentRevision === 0 || state.isBusy) {
    return;
  }

  // It throws away every edit at once, and there is no undoing it afterwards,
  // so it asks before it does.
  if (!window.confirm("開いたときの画像に戻します。これまでの編集は取り消せなくなります。よろしいですか？")) {
    return;
  }

  settleLastEdit();
  restoreImageData(state.documentPaper);
  state.undoStack = [];
  state.redoStack = [];
  ...
  updateHistoryButtons();
  updateDocumentState();
  setStatus("開いたときの画像に戻しました");
}

// An outline the drawing came with reads differently from paint of the same
// colour laid down since, and only the picture as it was opened says which is
// which. It is the same size as the canvas for as long as the document is open.
function openedPaper() {
  return state.documentPaper === null ? null : state.documentPaper.data;
}
...
async function loadImageFile(file) {
  ...
  try {
    ...
    // The picture as it was opened, kept whole and never written to. It is what
    // going back to the start restores, and what tells an outline the drawing
    // came with apart from paint laid over the same colours since.
    state.documentPaper = copyCurrentImageData();
    state.undoStack = [];
    state.redoStack = [];
    settleLastEdit();
    resetGuides();
    ...
    state.savedRevision = 0;
    state.nextRevision = 0;
    // One of those pictures is the opened one, which is held for as long as the
    // document is, so the history keeps one fewer.
    state.historyLimit = clamp(
      Math.floor(MAX_HISTORY_BYTES / (elements.canvas.width * elements.canvas.height * 4)) - 1,
      1,
      20,
    );
  ...
  runAfterNextPaint(() => {
    try {
      ...
      const result = editTool === "eraser"
        ? eraseContiguousRegion(before, point.x, point.y, editTolerance, guideBarrier, editWholePicture, editIncludeWhiteAndBlack, false)
        : fillContiguousRegion(before, point.x, point.y, cssColorToRgba(editColor), editTolerance, guideBarrier, editWholePicture, editIncludeWhiteAndBlack, true, openedPaper());
  ...
  runAfterNextPaint(() => {
    try {
      ...
      const result = edit.tool === "eraser"
        ? eraseContiguousRegion(before, edit.point.x, edit.point.y, tolerance, guideBarrier, wholePicture, includeWhiteAndBlack, false)
        : fillContiguousRegion(before, edit.point.x, edit.point.y, cssColorToRgba(edit.color), tolerance, guideBarrier, wholePicture, includeWhiteAndBlack, true, openedPaper());
...
elements.resetButton.addEventListener("click", resetToOpenedPicture);
